...
LOG_DIR = ROOT / "logs" / "auction_dp_weights_only_repro"
...
def run_regdp_weights_only(n: int, m: int, dist: str) -> dict[str, Any]:
    result, aux = mdp_dp.runtrial(
        num_agents=n,
        num_items=m,
        ...
        optimize_mode="weights_only",
        gamma=1.0,
    )
    result["vals_last"] = float(aux["vals"][-1]) if len(aux["vals"]) else None
    result["vals_best"] = float(max(aux["vals"])) if len(aux["vals"]) else None
    return result


def run_job(dist: str, n: int, m: int) -> None:
    method = "regdp_weights_only"
    ...
    result = run_regdp_weights_only(n, m, dist)
